...
class MDLRetriever(TopkRetriever):
    ...
    def cal_ce(self, input_texts: List[str], mask_length=None):
        if self.metric_model is None:
            ...
            # Determine appropriate dtype based on device capability
            if torch.cuda.is_available():
                # Use bfloat16 if supported (better numerical stability than float16)
            ...
            try:
                ...
                if not hasattr(self.metric_model, 'hf_device_map'):
                    self.metric_model.to(self.device)
            except Exception as e:
                logger.warning(f"Failed to load model with memory optimizations: {e}. Trying without device_map...")
            ...
            # Set to eval mode to save memory (no gradients needed)
        ...
        with torch.no_grad():
            ...
            if mask_length is not None:
                # Clamp mask_length to actual sequence length (after truncation)
                # mask_length is calculated from untruncated ICE, but tokenization may truncate
                actual_seq_len = loss.shape[-1]
                effective_mask_length = min(mask_length, actual_seq_len)
                
                if effective_mask_length < actual_seq_len:
                    mask = torch.cat([torch.zeros([loss.shape[0], effective_mask_length], dtype=torch.float),
                                      torch.ones([loss.shape[0], actual_seq_len - effective_mask_length], dtype=torch.float)], -1)
                else:
                    # If mask_length >= sequence length, don't mask anything (all zeros)
                    mask = torch.zeros([loss.shape[0], actual_seq_len], dtype=torch.float)
            ...
            if mask_length is not None:
                # Use effective_mask_length for lens calculation too
                effective_mask_length = min(mask_length, loss.shape[-1])
                lens -= effective_mask_length
                # Ensure lens is positive
